...
(sometimes many) most parsimonious dispersal histories \gb{in that case, how do we choose?}. We note that the 
...
substracting \gb{is this a typo?} the stem costs of $u$ and $v$ from the node costs of their
...
exponential dispersal kernel and 7 different standard deviation levels ($\sigma$)
that were equally spaced from 0.4\% to 4\% of the width of simulation landscape.
The Gaussian kernel emulates a Brownian motion while the double exponential 
kernel emulates a Laplace motion, which generates more extreme displacements of 
offspring relative to Brownian motion.
...
simulation was terminated after $4K = 2000$ generations \gb{that might not be long enough to reach stationary behavior in a spatial simulation}, after which the
...
\gb{do we do anything with nodes that show up across multiple marginal trees?}
